...
def tracer_resistance(df, source):
    ...
    df_plot["n"] = 8
    df_plot["event"] = df_plot["p"].rolling(window=8, min_periods=1).sum()
    df_plot["p_hat"] = df_plot["event"] / df_plot["n"]
    df_plot["SE"] = np.sqrt(df_plot["p_hat"] * (1 - df_plot["p_hat"]) / df_plot["n"])
    df_plot["upper"] = df_plot["p_hat"] + 1.96 * df_plot["SE"]
    df_plot["outlier"] = df_plot["p"] > df_plot["upper"]

    fig = px.line(df_plot, x="Week", y="p", markers=True, title=f"% hebdomadaire - {selected_ab}")
    ...
    fig.add_scatter(x=df_plot[df_plot["outlier"]]["Week"], y=df_plot[df_plot["outlier"]]["p"], mode="markers", name="Alerte", marker=dict(size=14, color="darkred", symbol="diamond"))
    fig.update_layout(xaxis_title="Semaine", yaxis_title="%", font=dict(size=16, color='black'))
...
with onglet[3]:
    ...
    for pheno in pheno_df.columns:
        if pheno.lower() != "week":
            st.markdown(f"### {pheno.upper()}")
            df_plot = pheno_df[["Week", pheno]].dropna()
            df_plot["p"] = df_plot[pheno] / df_plot.sum(axis=1)
            df_plot["n"] = 8
            df_plot["event"] = df_plot["p"].rolling(window=8, min_periods=1).sum()
            df_plot["p_hat"] = df_plot["event"] / df_plot["n"]
            df_plot["SE"] = np.sqrt(df_plot["p_hat"] * (1 - df_plot["p_hat"]) / df_plot["n"])
            ...
            fig = px.line(df_plot, x="Week", y="p", markers=True, title=f"% hebdomadaire - {pheno}")
            fig.update_traces(line=dict(width=4, color='black'))
            fig.add_scatter(x=df_plot["Week"], y=df_plot["upper"], mode="lines", name="Seuil", line=dict(dash="dot", color="red", width=3))
            fig.add_scatter(x=df_plot[df_plot["outlier"]]["Week"], y=df_plot[df_plot["outlier"]]["p"], mode="markers", name="Alerte", marker=dict(size=14, color="darkred", symbol="diamond"))
            fig.update_layout(xaxis_title="Semaine", yaxis_title="%", font=dict(size=16, color='black'))
            st.plotly_chart(fig, use_container_width=True)
